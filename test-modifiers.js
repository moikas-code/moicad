...
async function runTest(testCase) {
  ...
  try {
    ...
    if (!parseResult.success) {
      console.log(`❌ Parse failed: ${parseResult.errors?.map(e => e.message).join(', ')}`);
      return false;
    }
    ...
    if (testCase.expectGeometry === false) {
      ...
      if (!evalResult.success && evalResult.geometry === null) {
        ...
        console.log(`❌ Expected no geometry, but got: ${evalResult.success ? 'geometry' : 'evaluation failed'}`);
        return false;
      }
    }
    
    if (!evalResult.success) {
      console.log(`❌ Evaluation failed: ${evalResult.errors?.map(e => e.message).join(', ')}`);
      return false;
    }
    ...
    if (actualModifier === testCase.expectedModifier) {
      ...
      console.log(`❌ Modifier mismatch. Expected: ${testCase.expectedModifier || 'none'}, Got: ${actualModifier || 'none'}`);
      return false;
    }
...
async function main() {
  ...
  try {
    ...
    if (!healthResponse.ok) {
      console.log('❌ Server not running. Please start the moicad server first:');
      console.log('   bun --hot ./backend/index.ts');
      process.exit(1);
    }
    console.log('✅ Server is running\n');
  } catch (error) {
    console.log('❌ Cannot connect to server. Please start the moicad server first:');
    console.log('   bun --hot ./backend/index.ts');
    process.exit(1);
  }
