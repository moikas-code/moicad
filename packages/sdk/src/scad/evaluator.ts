...
if (result !== null) {
              geometries.push(result as ManifoldObject);
            }
      }
    } else {
      ...
      case "surface": {
        ...
        try {
          ...
          if (!fileResult.success || !fileResult.content) {
            throw new Error(fileResult.error || 'Failed to read surface file');
          }
...
class ParallelEvaluator {
  ...
  private createWorker(): void {
    // TODO: Implement worker for parallel evaluation - disabled for CommonJS
    return;
    // Worker code disabled for CommonJS compatibility
    /*
