...
const primitiveEvaluators: Record<string, PrimitiveEvaluator> = {
  ...
  square: (params, ctx) => {
    const rawSize = extractSize(params, 1);
    // For 2D square, only use first two dimensions
    const size: number | [number, number] = Array.isArray(rawSize)
      ? [rawSize[0], rawSize[1]]
      : rawSize;
    const center = getParam(params, ['center'], false);
...
export function toGeometry(manifold: ManifoldObject | ManifoldWithMeta): Geometry {
  if ('manifold' in manifold) {
    // ManifoldWithMeta - add metadata as extra property
    const geo = manifoldToGeometry(manifold.manifold);
    return { ...geo, ...({ metadata: manifold.metadata } as any) };
  }
