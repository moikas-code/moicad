...
export ALLOWED_ORIGINS="tauri://localhost,tauri://127.0.0.1,http://localhost:3000"
...
check_health() {
    ...
    for i in $(seq 1 $retries); do
        ...
        if pgrep -f "bun.*backend/index.ts" > /dev/null && curl -f -s --max-time 3 http://localhost:3000/health >/dev/null 2>&1; then
...
echo "📊 Ready check at: http://localhost:3000/ready"
echo "📊 Liveness probe at: http://localhost:3000/live"
...
if ! ps -p $SERVER_PID > /dev/null; then
    ...
    echo "   - Port 3000 is available: $(netstat -tuln | grep :3000 || echo 'Available')"
    exit 1
fi
