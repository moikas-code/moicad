...
export class MCPAIManager implements IAIManager {
  ...
  async setActiveProvider(providerId: string): Promise<void> {
    ...
    console.log(
      `Active AI provider set to: ${entry.provider.name} (${providerId})`,
    );
  }
  ...
  async generateSuggestions(
    ...
    try {
      ...
      console.log(
        `Generated ${suggestions.length} suggestions in ${Date.now() - startTime}ms`,
      );
  ...
  async validateSuggestion(
    ...
    try {
      ...
      const providerResult = await provider.validateSuggestion(
        suggestion,
        context,
      );

      // Add engine-level validation
      const engineResult = await suggestionEngine.validateSuggestion(
        suggestion,
        context,
      );
  ...
  async trackFeedback(
    ...
    try {
      ...
      console.log(
        `Tracked feedback for suggestion ${suggestionId}: ${feedback.rating}/5`,
      );
    } catch (error: any) {
  ...
  async getAnalytics(timeRange?: TimeRange): Promise<AIAnalytics> {
    try {
      ...
      const appliedSuggestions = filtered.filter(
        (s) => s.status === "applied",
      ).length;
      const rejectedSuggestions = filtered.filter(
        (s) => s.status === "rejected",
      ).length;

      const confidenceSum = filtered.reduce((sum, s) => sum + s.confidence, 0);
      const averageConfidence =
        totalSuggestions > 0 ? confidenceSum / totalSuggestions : 0;
      ...
      this.providers.forEach((entry, id) => {
        ...
        providerPerformance[id] = {
          ...
          userRating:
            perf.totalRatings > 0 ? perf.totalScore / perf.totalRatings : 0,
        };
      });
