...
export async function handleRegisterUser(req: Request): Promise<Response> {
  try {
    ...
    const existingUser =
      mcpStore.users.findByEmail(email) ||
      mcpStore.users.findByUsername(username);
    if (existingUser) {
    ...
    return jsonResponse(
      createErrorResponse("REGISTER_FAILED", "Failed to register user", {}, 500)
        .response,
      500,
    );
  }
...
export async function handleLoginUser(req: Request): Promise<Response> {
  try {
    ...
    return jsonResponse(
      createErrorResponse("LOGIN_FAILED", "Failed to login", {}, 500).response,
      500,
    );
  }
}
    ...
    projects = projects.filter(
      (project: Project) =>
        ...
        project.collaborators.some(
          (c: any) => c.userId === authContext.user!.id,
        ),
    );
  } else {
    ...
    sessions = sessions.filter(
      (session: Session) =>
        ...
        session.participants.some(
          (p: any) => p.userId === authContext.user!.id,
        ),
    );
  } else {
...
export async function handleGetSessionActivity(
  req: Request,
): Promise<Response> {
  const authContext = await authMiddleware(req);
  const url = new URL(req.url);
...
export async function handleGetConflictResolution(
  req: Request,
): Promise<Response> {
  const authContext = await authMiddleware(req);
  const url = new URL(req.url);
