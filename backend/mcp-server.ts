...
import type { 
  MCPMessage, MCPMessageType, User, Session, Project,
  SessionParticipant, ChangeDelta, ChatMessage, Suggestion,
  CursorPosition, ViewportState, SelectionRange, VersionEntry
...
export class MCPWebSocketServer {
  private messageHandlers = new Map<MCPMessageType, (message: MCPMessage, context: any) => Promise<MCPMessage | void>>();
  ...
  private setupMessageHandlers(): void {
    ...
    this.messageHandlers.set('session_leave', this.handleSessionLeave.bind(this));
    this.messageHandlers.set('session_create', this.handleSessionCreate.bind(this));
    ...
    this.messageHandlers.set('participant_join', this.handleParticipantJoin.bind(this));
    this.messageHandlers.set('participant_leave', this.handleParticipantLeave.bind(this));
    this.messageHandlers.set('participant_update', this.handleParticipantUpdate.bind(this));
    
    // Real-time editing
    this.messageHandlers.set('operation', this.handleOperation.bind(this));
    this.messageHandlers.set('operation_ack', this.handleOperationAck.bind(this));
    this.messageHandlers.set('operation_conflict', this.handleOperationConflict.bind(this));
    this.messageHandlers.set('cursor_update', this.handleCursorUpdate.bind(this));
    this.messageHandlers.set('selection_update', this.handleSelectionUpdate.bind(this));
    this.messageHandlers.set('viewport_update', this.handleViewportUpdate.bind(this));
    
    // Version control
    this.messageHandlers.set('version_create', this.handleVersionCreate.bind(this));
    this.messageHandlers.set('version_restore', this.handleVersionRestore.bind(this));
    ...
    this.messageHandlers.set('typing_indicator', this.handleTypingIndicator.bind(this));
    
    // Conflict resolution
    this.messageHandlers.set('conflict_resolve', this.handleConflictResolve.bind(this));
    this.messageHandlers.set('conflict_request', this.handleConflictRequest.bind(this));
    ...
    // Session recording
    this.messageHandlers.set('recording_start', this.handleRecordingStart.bind(this));
    this.messageHandlers.set('recording_stop', this.handleRecordingStop.bind(this));
    this.messageHandlers.set('recording_playback', this.handleRecordingPlayback.bind(this));
    
    // Geometry & evaluation
    this.messageHandlers.set('evaluate_request', this.handleEvaluateRequest.bind(this));
    this.messageHandlers.set('evaluate_cancel', this.handleEvaluateCancel.bind(this));
    this.messageHandlers.set('geometry_update', this.handleGeometryUpdate.bind(this));
    this.messageHandlers.set('export_request', this.handleExportRequest.bind(this));
    
    // AI assistance
    this.messageHandlers.set('suggestion_request', this.handleSuggestionRequest.bind(this));
    this.messageHandlers.set('suggestion_apply', this.handleSuggestionApply.bind(this));
    this.messageHandlers.set('suggestion_reject', this.handleSuggestionReject.bind(this));
  ...
  async handleMessage(message: MCPMessage, connectionId: string): Promise<void> {
  ...
  private async handleSessionJoin(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    if (!asAnonymous && context.user) {
      ...
      if (session.hostId !== user.id && !session.participants.some((p: any) => p.userId === user.id)) {
    ...
    const joinMessage: MCPMessage = {
      ...
      sessionId,
      payload: {
        participant: context.user ? session.participants.find((p: any) => p.userId === context.user.id) : null,
      },
    };
  ...
  private async handleSessionLeave(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    // Remove participant
    if (context.user) {
      session.participants = session.participants.filter((p: any) => p.userId !== context.user.id);
      mcpStore.sessions.set(sessionId, session);
  ...
  private async handleSessionCreate(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private async handleSessionUpdate(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private async handleSessionList(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    sessions = sessions.filter((session: Session) => 
      session.isActive && 
      (!context.user || session.hostId === context.user.id || session.participants.some((p: any) => p.userId === context.user.id))
    );
  ...
  private async handleParticipantJoin(message: MCPMessage, context: any): Promise<void> {
    // Handled in handleSessionJoin
  }
  
  private async handleParticipantLeave(message: MCPMessage, context: any): Promise<void> {
    // Handled in handleSessionLeave
  }
  
  private async handleParticipantUpdate(message: MCPMessage, context: any): Promise<MCPMessage> {
    const { sessionId, updates } = message.payload;
    ...
    const participant = session.participants.find((p: any) => p.userId === context.user?.id);
    if (!participant) {
  ...
  private async handleOperation(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    // Check permissions
    const participant = session.participants.find((p: SessionParticipant) => p.userId === context.user?.id);
    ...
    const documentState = mcpStore.getDocumentState(sessionId, fileId || 'default');
    ...
    const otOperation: Operation = {
      ...
      type: otType,
      position: operation.position?.line * 1000 + operation.position?.column || 0,
    ...
    const result = otManager.applyParticipantOperation(context.user.id, fileId || 'default', otOperation);
    ...
    const change: ChangeDelta = {
      ...
      type: result.transformed.type === 'retain' ? 'insert' : result.transformed.type as 'insert' | 'delete' | 'replace',
    ...
    session.sharedState.pendingChanges = session.sharedState.pendingChanges.filter((c: ChangeDelta) => c.id !== change.id);
    ...
    for (const p of session.participants) {
      if (p.userId !== context.user.id && p.cursor && p.cursor.file === (fileId || '')) {
    ...
    wsManager.broadcastToSession(sessionId, operationMessage, context.connectionId);
  ...
  private async handleCursorUpdate(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    wsManager.broadcastToSession(sessionId, cursorMessage, context.connectionId);
  ...
  private async handleViewportUpdate(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private async handleEvaluateRequest(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private async handleChatMessage(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private async handlePing(message: MCPMessage, context: any): Promise<MCPMessage> {
    return {
      id: mcpStore.generateId(),
      type: 'pong',
      timestamp: new Date(),
  ...
  private async handleAuthenticate(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private async handleVersionCreate(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    // Check permissions
    if (!context.user || (project.ownerId !== context.user.id && 
        !project.collaborators.some((c: any) => c.userId === context.user.id && c.role === 'owner'))) {
      throw new Error('No permission to create version');
    }
    
    const version = mcpStore.createVersionSnapshot(projectId, context.user.id, description);
  ...
  private async handleVersionRestore(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private async handleVersionList(message: MCPMessage, context: any): Promise<MCPMessage> {
    const { projectId } = message.payload;
  ...
  private async handlePresenceUpdate(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    wsManager.broadcastToSession(sessionId, presenceMessage, context.connectionId);
  ...
  private async handleTypingIndicator(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    wsManager.broadcastToSession(sessionId, typingMessage, context.connectionId);
  ...
  private async handleConflictResolve(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    const conflict = session.sharedState.pendingChanges.find((c: ChangeDelta) => c.id === conflictId);
    if (!conflict) {
  ...
  private async handleConflictRequest(message: MCPMessage, context: any): Promise<MCPMessage> {
    const { sessionId } = message.payload;
    ...
    const conflicts = session.sharedState.pendingChanges.filter((c: ChangeDelta) => c.conflicted);
  ...
  private async handleUndo(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private async handleRedo(message: MCPMessage, context: any): Promise<MCPMessage> {
    const { sessionId, fileId } = message.payload;
  ...
  private async handleRecordingStart(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    const recordingId = sessionRecorder.startRecording(sessionId, session.name, settings);
  ...
  private async handleRecordingStop(message: MCPMessage, context: any): Promise<MCPMessage> {
    ...
    return {
      ...
      timestamp: new Date(),
      sessionId,
      payload: { success: true, isRecording: false, recordingId: recording?.id },
    };
  }
  
  private async handleRecordingPlayback(message: MCPMessage, context: any): Promise<MCPMessage> {
  ...
  private generateUserColor(userId: string): string {
    const colors = [
      ...
      '#F8B739', '#52B788', '#E56B6F', '#A8DADC', '#457B9D'
    ];
  ...
  private async handleOperationAck(message: MCPMessage, context: any): Promise<void> {}
  private async handleOperationConflict(message: MCPMessage, context: any): Promise<void> {}
  private async handleSelectionUpdate(message: MCPMessage, context: any): Promise<void> {}
  private async handleGeometryUpdate(message: MCPMessage, context: any): Promise<void> {}
  private async handleExportRequest(message: MCPMessage, context: any): Promise<void> {}
  private async handleSuggestionRequest(message: MCPMessage, context: any): Promise<MCPMessage> {
    return await handleAIWebSocketMessage(message, context.connectionId) || {
      id: mcpStore.generateId(),
      type: 'error',
      timestamp: new Date(),
  ...
  private async handleSuggestionApply(message: MCPMessage, context: any): Promise<MCPMessage> {
    return await handleAIWebSocketMessage(message, context.connectionId) || {
      id: mcpStore.generateId(),
      type: 'error',
      timestamp: new Date(),
  ...
  private async handleSuggestionReject(message: MCPMessage, context: any): Promise<MCPMessage> {
    return await handleAIWebSocketMessage(message, context.connectionId) || {
      id: mcpStore.generateId(),
      type: 'error',
      timestamp: new Date(),
  ...
  private async handleEvaluateCancel(message: MCPMessage, context: any): Promise<void> {}
