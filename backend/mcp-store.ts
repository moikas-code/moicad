...
import type { 
  User, Project, Session, Suggestion, ChatMessage, 
  Invitation, ChangeDelta, EvaluationHistory, SystemEvent,
  VersionEntry, ProjectSnapshot, SessionParticipant
} from '../shared/mcp-types';
import { OTSessionManager, OTDocumentState, generateOperationId } from './mcp-operational-transform';
...
export class SuggestionStore extends InMemoryStore {
  findBySession(sessionId: string): Suggestion[] {
    return this.filter(suggestion => suggestion.timestamp.toString().includes(sessionId)); // Temporary hack
  }
...
export class ChatStore extends InMemoryStore {
  findBySession(sessionId: string): ChatMessage[] {
    return this.filter(message => message.sessionId === sessionId)
  ...
  findMessagesInDateRange(sessionId: string, start: Date, end: Date): ChatMessage[] {
...
export class ChangeStore extends InMemoryStore {
  findBySession(sessionId: string): ChangeDelta[] {
    return this.filter(change => 
      change.position.file.includes(sessionId) // Temporary hack
    ).sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
  }
...
export class EvaluationStore extends InMemoryStore {
  ...
  findBySession(sessionId: string): EvaluationHistory[] {
    return this.filter(evalution => 
      evalution.code.includes(sessionId) // Temporary hack
    );
  }
  ...
  findInDateRange(start: Date, end: Date): EvaluationHistory[] {
    return this.filter(evalution => 
      evalution.timestamp >= start && evalution.timestamp <= end
    );
  }
}
...
export class VersionStore extends InMemoryStore {
  findByProject(projectId: string): VersionEntry[] {
    return this.filter(version => 
      version.snapshot && version.snapshot.files // Check if projectId can be derived
  ...
  findInRange(projectId: string, startVersion: number, endVersion: number): VersionEntry[] {
    return this.filter(version => 
      version.version >= startVersion && 
      version.version <= endVersion &&
      version.snapshot && version.snapshot.files
...
export class MCPStore {
  ...
  cleanupExpired(): void {
    ...
    console.log(`Cleaned ${expiredSessions.length} sessions, ${expiredSuggestions.length} suggestions, ${expiredInvitations.length} invitations`);
  }
  ...
  updateParticipantCursor(
    ...
    const participant = session.participants.find((p: SessionParticipant) => p.userId === participantId);
    if (participant) {
  ...
  updateParticipantSelection(
    sessionId: string,
    participantId: string,
    selection: { start: { line: number; column: number; file: string }; end: { line: number; column: number; file: string } }
  ): void {
    const session = this.sessions.get(sessionId);
    if (!session) return;
   ...
   */
  getSessionActivity(sessionId: string, timeRange?: { start: Date; end: Date }): {
    ...
    const operationCount = this.changes.filter(change => 
      change.timestamp >= start && change.timestamp <= end
    ).length;
    
    const messageCount = this.chats.filter(message => 
      message.sessionId === sessionId &&
      message.timestamp >= start && message.timestamp <= end
    ).length;
    
    const evaluationCount = session.sharedState.evaluations.filter(evalution => 
      evalution.timestamp >= start && evalution.timestamp <= end
    ).length;
