...
import {
  OTSessionManager,
  generateOperationId,
} from "./backend/mcp-operational-transform";
...
operations.forEach((op, index) => {
  console.log(
    `   ${index + 1}. User ${op.authorId}: ${op.type} "${op.content || `${op.length} chars`}" at position ${op.position}`,
  );
});
...
operations.forEach((operation) => {
  ...
  console.log(
    `   → ${result.success ? "✅ Applied" : "❌ Failed"}${result.conflicts.length > 0 ? ` with ${result.conflicts.length} conflicts` : ""}`,
  );
});
...
if (recording) {
  ...
  console.log(
    `   Max concurrent participants: ${recording.metadata.maxConcurrentParticipants}\n`,
  );
}
...
if (analysis) {
  ...
  console.log(
    `   Total Duration: ${(analysis.summary.totalDuration / 1000).toFixed(2)}s`,
  );
  console.log(
    `   Active Time: ${(analysis.summary.activeTime / 1000).toFixed(2)}s`,
  );
  console.log(
    `   Idle Time: ${(analysis.summary.idleTime / 1000).toFixed(2)}s`,
  );
  ...
  analysis.participants.forEach((participant) => {
    ...
    console.log(
      `     Operations: ${participant.contributionMetrics.operationsCount}`,
    );
    console.log(
      `     Messages: ${participant.contributionMetrics.messageCount}`,
    );
    console.log(
      `     Active Time: ${participant.contributionMetrics.activeTimePercentage.toFixed(1)}%`,
    );
    console.log(
      `     Collaboration Score: ${participant.contributionMetrics.collaborationScore.toFixed(1)}/100`,
    );
  });
  ...
  console.log(
    `   Collaboration Style: ${analysis.patterns.collaborationStyle}`,
  );
  console.log(`   Conflict Frequency: ${analysis.patterns.conflictFrequency}`);
  console.log(
    `   Communication Pattern: ${analysis.patterns.communicationPattern}\n`,
  );
}
...
console.log(
  `   Languages: ${Object.entries(version.snapshot.metadata.languageCounts)
...
console.log(
  `   User 1: Insert "${conflictingOp1.content}" at position ${conflictingOp1.position}`,
);
console.log(
  `   User 2: Insert "${conflictingOp2.content}" at position ${conflictingOp2.position}`,
);
...
if (hasConflict) {
  ...
  strategies.forEach((strategy) => {
    ...
    console.log(
      `     Winner: ${resolution.resolved.authorId === users[0].id ? users[0].displayName : users[1].displayName}`,
    );
    console.log(
      `     Resolved content: "${resolution.resolved.content || "N/A"}"`,
    );
  });
}
...
console.log("\nThen connect WebSocket clients to: ws://localhost:42069/ws");
