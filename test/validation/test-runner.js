...
async function runTest(name, code, expectedType = 'success') {
  ...
  try {
    ...
    if (expectedType === 'success' && !evalResult.success) {
      ...
      TEST_RESULTS.failed++;
    } else if (evalResult.success && (!evalResult.geometry || evalResult.geometry.stats.vertexCount === 0)) {
      status = '⚠️ PARTIAL';
    ...
    if (result.geometry) {
      console.log(`    📊 Geometry: ${result.geometry.vertices} vertices, ${result.geometry.faces} faces`);
    }
...
async function loadTestCases(category) {
  if (category === 'quick-validation') {
    const content = await import(path.join(__dirname, 'categories', 'quick-validation.js'));
...
async function main() {
  ...
  if (categoriesToRun.length === 0) {
    console.log('Usage: node test-runner.js [--all | --category=<number|name>] [--verbose]');
    console.log('Categories:', Object.entries(TEST_CATEGORIES).map(([k,v]) => `${k}=${v}`).join(', '));
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('❌ Server is not responding. Please start with: bun --hot ./backend/index.ts');
    process.exit(1);
  }
  ...
  const total = TEST_RESULTS.passed + TEST_RESULTS.failed + TEST_RESULTS.partial + TEST_RESULTS.notTested;
  const successRate = total > 0 ? ((TEST_RESULTS.passed / total) * 100).toFixed(1) : 0;
